...
router.post('/api/order', async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (orderRes?.affectedRows > 0) {
      res.send({
        response_status: 1,
        message: 'Order created successful',
      });
      pushToQueue("successful", originName, id);
    } else {
      throw Error("Order failure")
    }
  } catch (error) {
    res.send({
      response_status: 1,
      message: 'Order created failure',
    });
    pushToQueue("failure", originName, id);
  }
